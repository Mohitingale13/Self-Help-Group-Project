...
export default function DashboardScreen() {
  ...
  useSpeechRecognitionEvent("result", (event) => {
    console.log("[SpeechRecognition] result event fired:", JSON.stringify(event));
    if (Platform.OS === "web") return;
    const text = event.results?.[0]?.transcript ?? "";
    if (text) {
      micStateRef.current = "processing";
      console.log("[SpeechRecognition] ref set to:", micStateRef.current);
      finishWithResult(text);
    }
  });

  useSpeechRecognitionEvent("error", (event) => {
    console.log("[SpeechRecognition] error event fired:", JSON.stringify(event));
    if (Platform.OS === "web") return;
  ...
  useSpeechRecognitionEvent("end", () => {
    console.log("[SpeechRecognition] end event fired. micState at end:", micStateRef.current);
    if (Platform.OS === "web") return;
